...
def predict_for_video(video_path, model, args):
    ...
    for i, (rgb, flow) in enumerate(segments):
        ...
        with torch.no_grad():
            ...
            print(f"📦 Segment {i+1:02d}: 이상 확률 {prob * 100:.2f}% {'🔥' if label == 1 else '✅'}")
    ...
    csv_path = os.path.join(CSV_OUTPUT_DIR, f"{video_name}.csv")

    with open(csv_path, mode="w", newline="") as csv_file:
        writer = csv.writer(csv_file)
        writer.writerow(["frame", "anomaly"])
        for idx, val in enumerate(frame_results):
...
def run_batch():
    ...
    for file in os.listdir(VIDEO_DIR):
        if file.lower().endswith((".mp4", ".avi", ".mov", ".mkv")):
            ...
            predict_for_video(video_path, model, args)
